...
def handle_callback(callback):
    ...
    if data.startswith("tariff_"):
        ...
        if not promo:
            edit_message(chat_id, msg_id, "❌ Промо-коды кончились. Напишите @Sheizik", {"inline_keyboard": [[{"text": "🆘 Поддержка", "callback_data": "support"}]]})
            answer_callback(callback["id"])
            return
    ...
    # Админские действия (упрощённо, без FSM — через отдельные сообщения)
    if data == "add_promo":
        if chat_id not in ADMIN_IDS:
            answer_callback(callback["id"], "⛔ Нет доступа", True)
            return
        # В реальности тут надо запросить ввод — проще через команду /addpromo (см. ниже)
    ...
    if data == "admin_stats":
        if chat_id not in ADMIN_IDS:
            ...
            return
        total_users, total_purchases, total_stars, free_promos = get_stats()
        text = f"📈 *Полная статистика:*\n👥 {total_users}\n💳 {total_purchases}\n⭐ {total_stars}\n🎫 Всего промо: {get_all_promos(9999)} (свободных: {free_promos})"
        edit_message(chat_id, msg_id, text, back_button())
        answer_callback(callback["id"])
        return
