...
# ============================================
# .DEL — УДАЛЯЕТ СООБЩЕНИЕ В ЛИЧКЕ (через ссылку)
# ============================================
def del_msg(update: Update, context: CallbackContext):
    ...
    # Ссылка на удаление через Bot API
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/deleteMessage?chat_id={chat_id}&message_id={msg_id}"
    
    # Отправляем ссылку админу
    update.message.reply_text(
        f"🗑️ **УДАЛИТЬ СООБЩЕНИЕ**\n\n"
        f"👤 {get_user_link(target_user)}\n"
        f"📝 {target.text or '[медиа]'}\n\n"
        f"👉 [НАЖМИ СЮДА, ЧТОБЫ УДАЛИТЬ]({url})\n\n"
        f"⚠️ Сообщение удалится у ОБОИХ!",
        parse_mode="Markdown",
...
def mute(update: Update, context: CallbackContext):
    ...
    if not update.message.reply_to_message:
        update.message.reply_text(
            ...
            "2️⃣ Напиши .mute\n\n"
            "Бот будет удалять его сообщения!",
            parse_mode="Markdown"
        )
        return
...
def main_menu(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        ...
        "3️⃣ Нажми на ссылку!\n\n"
        "**ДРУГИЕ КОМАНДЫ:**\n"
        ".mute .unmute .warn .save .spam .coin",
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if result:
        ...
        try:
            update.message.delete()
            print(f"🗑️ Удалено сообщение от {user_id}")
...
def main():
    ...
    print("✅ .del — УДАЛЯЕТ СООБЩЕНИЯ В ЛИЧКЕ!")
