...
# ===== ПАРСИНГ TIKTOK (БЕСПЛАТНЫЙ) =====
async def tiktok_search(query):
    """Ищет TikTok аккаунты, видео, комментарии, номера"""
    result = {
        "social": [],
        "personal": {},
        "comments": [],
    ...
    nick = query.strip()
    # Аккаунт по нику
    result["social"].append(f"🎵 TikTok (профиль): https://www.tiktok.com/@{nick}")

    # Поиск по нику через Google
    google_tiktok = f"https://www.google.com/search?q=site:tiktok.com+{nick}"
    result["social"].append(f"🔍 Google (TikTok): {google_tiktok}")

    # Комментарии через публичный RSS
    try:
        rss_url = f"https://www.tiktok.com/@{nick}"
        r = requests.get(rss_url, timeout=5)
        if r.status_code == 200:
            result["comments"].append(f"📌 Комментарии: https://www.tiktok.com/@{nick}/video")
    except:
        pass

    # Поиск номера в TikTok (через Google Dork)
    if len(nick) > 5:
        phone_dork = f"https://www.google.com/search?q=%22{nick}%22+OR+%22@{nick}%22+site:tiktok.com+OR+site:vm.tiktok.com"
        result["social"].append(f"📱 Возможные номера/контакты: {phone_dork}")

    # Поиск через Яндекс
    yandex_tiktok = f"https://yandex.ru/search/?text={nick}+site:tiktok.com"
    result["social"].append(f"🔍 Яндекс (TikTok): {yandex_tiktok}")

    # Дополнительные методы
    result["social"].append(f"🔍 Социальные поиск: https://www.tiktok.com/search?q={nick}")
...
async def smart_search(query):
    ...
    # --- TikTok ---
    if parsed["nick"] or parsed["name"]:
        tiktok_data = await tiktok_search(parsed["nick"] or parsed["name"])
        result["social"].extend(tiktok_data["social"])
        result["comments"].extend(tiktok_data["comments"])
        result["phones"].extend(tiktok_data["phones"])
...
'''
    ...
    html += f'''
    <div class="footer">Источники: VK API, Google, Яндекс, HaveIBeenPwned, ip-api.com, TikTok, Instagram, GitHub, Twitter</div>
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• 📱 Номер телефона\n"
        f"• 📧 Email\n"
        f"• 👤 ФИО + адрес, город, страна, возраст\n"
        f"• 🆔 Никнейм (TikTok, Instagram, VK, GitHub, Twitter)\n\n"
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            ...
            "• 📱 Номер телефона\n"
            "• 📧 Email\n"
            "• 👤 ФИО + адрес, город, страна, возраст\n"
            "• 🆔 Никнейм (TikTok, Instagram, VK, GitHub, Twitter, Telegram)\n\n"
        ...
        await query.edit_message_text(
            ...
            "👤 *ФИО + адрес, город, страна, возраст*\n"
            "→ Поиск в VK, Google, Яндекс, соцсетях\n\n"
            "🆔 *Никнейм*\n"
            "→ Поиск в TikTok, VK, Instagram, GitHub, Twitter, Telegram\n\n"
