...
# ===== НАСТОЯЩИЙ ПОИСК (РАБОТАЕТ) =====
def search_wikipedia(query):
    """Поиск в Wikipedia с обходом блокировки"""
    try:
        headers = {
...
def search_yandex(query):
    """Поиск через Яндекс (мобильная версия)"""
    try:
        headers = {
        ...
        if r.status_code == 200:
            ...
            if alt_match:
                text = re.sub(r'<.*?>', '', alt_match.group(1))
                return text[:300]
    except:
        pass
    return None
...
def search_google(query):
    """Поиск через Google (мобильная версия)"""
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Linux; Android 11; SM-G973F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36",
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
...
def get_person_info(query):
    """Собирает биографию человека из всех источников"""
    # 1. Wikipedia (самый надёжный)
    info = search_wikipedia(query)
    if info:
        return {"source": "Wikipedia", "text": info}
    
    # 2. DuckDuckGo
    info = search_ddg(query)
    if info:
        return {"source": "DuckDuckGo", "text": info}
    
    # 3. Яндекс
    info = search_yandex(query)
    if info:
        return {"source": "Яндекс", "text": info}
    
    # 4. Google
    info = search_google(query)
    if info:
        return {"source": "Google", "text": info}
...
async def smart_search(query, user_id):
    ...
    if person_info:
        ...
        result["personal"]["Источник"] = person_info["source"]
