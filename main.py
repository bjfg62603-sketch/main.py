...
# ============================================
# .MUTE — ДОБАВЛЯЕТ В ЧЁРНЫЙ СПИСОК (АВТОУДАЛЕНИЕ)
# ============================================
...
# ============================================
# .UNMUTE — УБИРАЕТ ИЗ ЧЁРНОГО СПИСКА
# ============================================
...
def save_message(update: Update, context: CallbackContext):
    ...
    c.execute("INSERT INTO saved (chat_id, user_id, file_id, file_type, caption, date) VALUES (?, ?, ?, ?, ?, datetime('now'))",
...
def main_menu(update: Update, context: CallbackContext):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text(
        "🌟 **SMERTNYTEAM BOT V8.0**\n\n"
        "🔥 **УДАЛЕНИЕ В ЛИЧКЕ РАБОТАЕТ!**\n\n"
        "**КАК УДАЛИТЬ ОДНО СООБЩЕНИЕ:**\n"
        "1️⃣ Ответь на сообщение\n"
        "2️⃣ Напиши .del\n\n"
        "**КАК ЗАМУТИТЬ (АВТОУДАЛЕНИЕ):**\n"
        "1️⃣ Ответь на сообщение\n"
        "2️⃣ Напиши .mute\n"
        "3️⃣ Бот будет удалять ВСЕ его сообщения!\n\n"
        "**ДРУГИЕ КОМАНДЫ:**\n"
...
def button_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "back_menu":
        ...
        query.edit_message_text(
            ...
            "**.mute** — автоудаление всех сообщений",
            reply_markup=reply_markup,
            parse_mode="Markdown"
        )
        return
    ...
    commands_info = {
        "cmd_del": "🗑️ **УДАЛИТЬ ОДНО**\nОтветь на сообщение → .del",
        "cmd_mute": "🔇 **МУТ (АВТОУДАЛЕНИЕ)**\nОтветь на сообщение → .mute\nБот будет удалять ВСЕ сообщения этого человека",
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if result:
        ...
        try:
            update.message.delete()
            print(f"🗑️ АВТОУДАЛЕНИЕ: сообщение от {user_id} в чате {chat_id}")
        except Exception as e:
...
def main():
    ...
    print("✅ .del — УДАЛЯЕТ ОДНО СООБЩЕНИЕ")
    print("✅ .mute — АВТОУДАЛЕНИЕ ВСЕХ СООБЩЕНИЙ")
