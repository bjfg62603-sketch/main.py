...
# ===== ИИ-ПОИСК (ЧЕРЕЗ БЕСПЛАТНЫЙ API) =====
def ai_search(query):
    """Использует ИИ для поиска информации"""
    try:
        # Пытаемся использовать DuckDuckGo API (он же даёт описание)
    ...
    # Если не сработало — используем Wikipedia
    try:
...
def get_person_info(query):
    """Собирает биографию человека: ФИО, дата рождения, место, возраст, карьера"""
    info = ai_search(query)
    if not info:
        return None
    ...
    # Пытаемся вытащить структурированные данные
    result = {
        "full_name": query,
        "bio": info[:500] + "..." if len(info) > 500 else info
    }
    
    # Ищем дату рождения
    birth_match = re.search(r'(\d{1,2}\s+(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)\s+\d{4})', info)
    if birth_match:
        result["birth_date"] = birth_match.group(1)
    
    # Ищем возраст
    age_match = re.search(r'(\d{1,3})\s+(год|года|лет)', info)
    if age_match:
        result["age"] = age_match.group(1)
    
    # Ищем место рождения
    place_match = re.search(r'(родился|род\.|р\.)\s+в\s+([^.,]+)', info)
    if place_match:
        result["birth_place"] = place_match.group(2).strip()
...
async def smart_search(query, user_id):
    ...
    # --- Поиск человека (ФИО) ---
    # 1. Пытаемся получить биографию через ИИ
    person_info = get_person_info(query)
    if person_info:
        result["bio"] = person_info
        if person_info.get("full_name"):
            result["personal"]["ФИО"] = person_info["full_name"]
        if person_info.get("birth_date"):
            result["personal"]["Дата рождения"] = person_info["birth_date"]
        if person_info.get("age"):
            result["personal"]["Возраст"] = person_info["age"]
        if person_info.get("birth_place"):
            result["personal"]["Место рождения"] = person_info["birth_place"]
        if person_info.get("bio"):
            result["description"] = person_info["bio"]
    ...
    if not result["bio"] and not result["social"]:
...
'''
    if data.get("bio"):
        html += f'    <div class="item">🧠 ИИ: {data["bio"]["bio"]}</div>\n'
...
'''
    ...
    html += f'''
    <div class="footer">Источники: ИИ (DuckDuckGo, Wikipedia), Google, Яндекс, VK API, HaveIBeenPwned, ip-api.com</div>
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.get("bio"):
            bio = data["bio"]
            if "full_name" in bio:
                msg += f"👤 ФИО: {bio['full_name']}\n"
            if "birth_date" in bio:
                msg += f"📅 Дата рождения: {bio['birth_date']}\n"
            if "age" in bio:
                msg += f"📊 Возраст: {bio['age']}\n"
            if "birth_place" in bio:
                msg += f"📍 Место рождения: {bio['birth_place']}\n"
            if "bio" in bio:
                msg += f"📝 {bio['bio']}\n\n"
        if data.get("phone"):
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🧠 Бот использует ИИ для поиска биографий.\n\n"
        f"📌 Пример: `Павел Дуров` или `дай биографию Путина`",
        reply_markup=main_menu(),
        parse_mode="Markdown"
    )
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            "✅ *Чекки*\n\n"
            "• ФИО — биография через ИИ, соцсети\n"
...
print("🚀 Smertnyy Search с ИИ запущен")
app.run_polling()
