...
async def real_search(query):
    """Реальный поиск по открытым источникам"""
    result = {
        "personal": {},
        "phone": None,
        "email": None,
    ...
    # --- 1. Поиск во ВКонтакте (через публичный API) ---
    try:
        vk_api_url = f"https://api.vk.com/method/users.search?q={query}&count=5&v=5.131"
        ...
        if r.status_code == 200:
            ...
            if "response" in data:
                for item in data["response"]["items"]:
                    ...
                    result["vk_data"].append(vk_link)
                    result["social"].append(f"✅ VK: {vk_link}")
                    if "first_name" in item and "last_name" in item:
                        result["personal"]["Имя"] = f"{item['first_name']} {item['last_name']}"
    except:
        pass

    # --- 2. Поиск в Google и Яндекс ---
    google_query = f"https://www.google.com/search?q={query.replace(' ', '+')}"
    yandex_query = f"https://yandex.ru/search/?text={query.replace(' ', '+')}"
    result["google_links"].append(f"Google: {google_query}")
    result["google_links"].append(f"Яндекс: {yandex_query}")

    # --- 3. Если в запросе есть номер телефона ---
...
'''
    ...
    html += f'''
    <div class="footer">Источники: VK API, Google, Яндекс, HaveIBeenPwned</div>
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📌 Отправьте номер телефона, ФИО или Email для поиска.\n\n"
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            ...
            "• 📧 Email\n"
            "• 👤 ФИО или ник\n\n"
            "Я проверю утечки, соцсети и открытые источники.",
        ...
        await query.edit_message_text(
            ...
            "👤 *ФИО или ник*\n"
            "→ Поиск в VK, Instagram, GitHub, Twitter\n\n"
            "🔍 *Google / Яндекс*\n"
