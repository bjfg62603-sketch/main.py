...
def parse_query(query):
    ...
    # --- Город (по ключевым словам) ---
    city_keywords = ["город", "г.", "city", "в", "из", "живет", "проживает"]
    ...
    # --- Страна (по ключевым словам) ---
    country_keywords = ["страна", "country", "гражданин", "из"]
    ...
    if address_match:
        # Захватываем весь адрес до конца строки или до ключевого слова
    ...
    # --- Оставшийся текст — ФИО или ник ---
    clean_query = re.sub(r'[^\w\s]', '', query)
    clean_query = re.sub(r'\s+', ' ', clean_query).strip()
    words = clean_query.split()

    # Если осталось 2-4 слова с заглавными буквами — это ФИО
...
async def smart_search(query):
    """Умный поиск на основе распарсенных данных"""
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    result["personal"]["Организация"] = data.get("org", "Неизвестно")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.get("personal"):
            for k, v in data["personal"].items():
                if k not in ["IP", "Город", "Регион", "Страна", "Провайдер", "Организация"]:
                    msg += f"👤 {k}: {v}\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• Номер телефона\n"
        f"• Email\n"
        f"• ФИО + адрес, город, страна, возраст (любая комбинация)\n"
        f"• Никнейм\n\n"
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            ...
            f"📊 Осталось запросов: {balance}\n\n"
            f"🔍 Бот ищет информацию по утечкам и открытым источникам.\n"
            f"📌 Отправьте номер телефона, ФИО, Email, IP или ник для поиска.\n\n"
            f"Используйте кнопки меню:",
            reply_markup=main_menu(),
            parse_mode="Markdown"
        )
        ...
        await query.edit_message_text(
            ...
            "• 📱 Номер телефона\n"
            "• 📧 Email\n"
            "• 👤 ФИО + адрес, город, страна, возраст (любая комбинация)\n"
            "• 🆔 Никнейм\n\n"
        ...
        await query.edit_message_text(
            ...
            "🆔 *Никнейм*\n"
            "→ Поиск в VK, Instagram, GitHub, Twitter, Telegram\n\n"
