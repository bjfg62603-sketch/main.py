...
# ============================================
# ГЛАВНАЯ КОМАНДА .mute (РАБОТАЕТ В ЛИЧКЕ!)
# ============================================
def mute(update: Update, context: CallbackContext):
    ...
    # Если это НЕ ЛС (группа) — проверяем ответ
    if chat_id != user_id:
    ...
    # Сохраняем в БД (chat_id = user_id, т.к. это ЛС с админом)
    c.execute("INSERT OR REPLACE INTO muted VALUES (?, ?, ?)", (user_id, target_id, until))
    conn.commit()

    # Удаляем сообщение с командой (.mute)
    try:
        update.message.delete()
    except:
        pass
...
# ============================================
# КОМАНДА .unmute (РАБОТАЕТ В ЛИЧКЕ!)
# ============================================
...
def warn(update: Update, context: CallbackContext):
    ...
    c.execute("INSERT INTO warns (chat_id, user_id, reason, date) VALUES (?, ?, ?, datetime('now'))",
              (user_id if chat_id == user_id else chat_id, target_id, reason))
    conn.commit()

    c.execute("SELECT COUNT(*) FROM warns WHERE chat_id=? AND user_id=?", 
              (user_id if chat_id == user_id else chat_id, target_id))
...
def main_menu(update: Update, context: CallbackContext):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text(
        "🌟 **SmertnyTeam Bot v3.0**\n\n"
        "🔥 Режим секретаря **ВКЛЮЧЁН**!\n\n"
        "📌 **КАК ИСПОЛЬЗОВАТЬ:**\n"
        "1️⃣ Ответь на **сообщение человека**\n"
        "2️⃣ Напиши `.mute` (или `/mute`)\n"
        "3️⃣ Бот сделает всё сам!\n\n"
        "📌 **ДРУГИЕ КОМАНДЫ:**\n"
        "`.unmute` — размутить\n"
        "`.save` — сохранить\n"
        "`.warn` — выдать варн\n"
...
def button_callback(update: Update, context: CallbackContext):
    ...
    commands_info = {
        ...
        "cmd_warn": "⚠️ **Варн**\n\nОтветь на сообщение → `.warn причина`\nПосле 5 варнов — авто-мут",
    ...
    if query.data == "warns_list":
        ...
        keyboard = [[InlineButton("🔙 Назад в админку", callback_data="admin_back")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        query.edit_message_text(text, reply_markup=reply_markup, parse_mode="Markdown")
        return
