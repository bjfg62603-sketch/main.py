...
# ==========================
# 4. РАСШИРЕННЫЙ ПОИСК (Аналог платных API)
# ==========================
...
def search_vk_api(query: str) -> List[str]:
    result = []
    try:
...
def search_google_dorks(query: str) -> List[str]:
    result = []
    try:
        dorks = [
            f"\"{query}\" site:pastebin.com",
            f"\"{query}\" site:github.com",
            f"\"{query}\" site:telegram.org",
            f"\"{query}\" site:vk.com",
        ...
        for dork in dorks:
            ...
            if r.status_code == 200:
                result.append(f"🔍 Google Dork: {url}")
                break
    except:
        pass
    return result
...
async def smart_search(query: str, user_id: int) -> Dict:
    ...
    if len(query.split()) >= 2:
        vk_results = search_vk_api(query)
        if vk_results:
            result["social"].extend(vk_results)
        social_results = search_social_media(query.replace(" ", "_"))
        if social_results:
            result["social"].extend(social_results)
        google_results = search_google_dorks(query)
        if google_results:
            result["social"].extend(google_results)
        return result
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🔍 Введите:\n"
        f"• 👤 ФИО — полный пробив по ЕГРН + соцсети\n"
        f"• 📱 Номер телефона — соцсети\n"
        f"• 🆔 Ник Telegram — ID и соцсети\n"
        f"• 🌐 IP-адрес — геолокация\n"
        f"• 📧 Email — утечки\n\n"
        f"📌 Бот использует: базу ЕГРН, VK, Telegram, Instagram, GitHub, Google Dorks, HaveIBeenPwned, ip-api.com",
