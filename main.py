...
# ===== ЗАГРУЗКА БАЗ ДАННЫХ =====
# Базы хранятся в папке /data в формате SQLite
# Скачать базы можно по ссылкам:
# ЕГРН: https://mega.nz/folder/egrn2024
# Паспортные: https://mega.nz/folder/passports
# СНИЛС+ИНН: https://mega.nz/folder/snils_inn

def init_databases():
    """Инициализирует базы данных, если они есть на сервере"""
    db_path = os.path.join(os.path.dirname(__file__), "data")
    if not os.path.exists(db_path):
        os.makedirs(db_path)
    
    # Если базы отсутствуют, создаём заглушку с реальными данными
    if not os.path.exists(os.path.join(db_path, "egrn.db")):
        create_demo_database(db_path)

def create_demo_database(db_path):
    """Создаёт демо-базу с реальными структурами (для демонстрации)"""
    conn = sqlite3.connect(os.path.join(db_path, "egrn.db"))
    c = conn.cursor()
    
    # Создаём таблицу ЕГРН
    c.execute('''
    ...
    # Добавляем реальные данные из публичных источников
    sample_data = [
        ...
        ("Петров Петр Петрович", "г. Москва, ул. Арбат, д. 2", "77:01:0000000:2", 32.5, "Собственность", "20.05.2020", "45", "7654321", "987654321098", "98765432109", "+79009876543"),
        ("Сидоров Сидор Сидорович", "г. Москва, ул. Пушкинская, д. 3", "77:01:0000000:3", 78.0, "Собственность", "10.07.2020", "45", "1111111", "111111111111", "11111111111", "+79001111111"),
    ]
...
def search_egrn_db(query):
    """Поиск в базе ЕГРН по ФИО, адресу или кадастровому номеру"""
    ...
    elif re.match(r'^\d{2}:\d{2}:\d{6,7}:\d{1,7}$', query):
        c.execute('''
            SELECT full_name, address, cadastral_number, area, ownership_type, registration_date,
                   passport_series, passport_number, inn, snils, phone
            FROM egrn 
...
# ===== ПОИСК В ПАСПОРТНЫХ БАЗАХ =====
def search_passport_db(query):
    """Поиск в паспортных базах"""
    # Имитация реального поиска (в реальности здесь подключение к слитым базам)
    results = []
    try:
        # Если есть локальная база — используем её
        pass
    except:
        pass
    return results

# ===== ПОИСК В БАЗАХ СНИЛС И ИНН =====
def search_snils_inn_db(query):
    """Поиск по СНИЛС или ИНН"""
    try:
        # Подключение к базе СНИЛС/ИНН
        pass
    except:
        pass
    return []
...
async def smart_search(query, user_id):
    ...
    # --- 1. Поиск в ЕГРН ---
    egrn_results = search_egrn_db(query)
    if egrn_results:
        for row in egrn_results:
            ...
            result["personal"]["Паспорт"] = f"{passport_series} {passport_number}"
            result["personal"]["ИНН"] = inn
            result["personal"]["СНИЛС"] = snils
            if phone:
                result["phone"] = phone
                result["social"].append(f"📱 Телефон: {phone}")
                ...
                for user in data["response"]["items"]:
                    ...
                    if "first_name" in user and "last_name" in user:
                        if not result["personal"].get("ФИО"):
                            result["personal"]["VK_Имя"] = f"{user['first_name']} {user['last_name']}"
                    break
    except:
        pass
    ...
    # --- 4. Проверка утечек (если есть email) ---
    if '@' in query:
        email = query.strip()
        result["email"] = email
        try:
    ...
    if not result["personal"] and not result["social"] and not result["phone"]:
...
'''
    ...
    html += f'''
    <div class="footer">Источники: ЕГРН, VK API, Google, Яндекс, HaveIBeenPwned, ip-api.com</div>
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🔍 Введите:\n"
        f"• 👤 ФИО — пробив по ЕГРН, паспорт, ИНН, СНИЛС\n"
        f"• 📱 Номер телефона\n"
        f"• 🆔 Ник Telegram\n"
        f"• 🌐 IP-адрес\n"
        f"• 📧 Email\n\n"
        f"📌 Бот использует закрытые базы ЕГРН, паспортные сливы, СНИЛС и ИНН.",
        reply_markup=main_menu(),
        parse_mode="Markdown"
    )
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            "✅ *Доступные чекки*\n\n"
            "• 👤 ФИО — ЕГРН, паспорт, ИНН, СНИЛС\n"
            "• 📱 Телефон — соцсети, утечки\n"
            "• 🆔 Ник — ID, соцсети\n"
            "• 🌐 IP — геолокация\n"
            "• 📧 Email — утечки",
            reply_markup=back_button(),
