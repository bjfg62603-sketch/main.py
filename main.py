...
def check_account_exists(url):
    """Проверяет, существует ли аккаунт по ссылке"""
    try:
...
async def smart_search(query):
    ...
    if phone_match:
        ...
        # Проверяем утечки через HaveIBeenPwned (по номеру)
        try:
    ...
    # --- Поиск по ФИО или нику ---
    clean_query = re.sub(r'[^\w\s]', '', query).strip()
    nick = clean_query.replace(" ", "_")

    # 1. Проверяем VK
    vk_url = f"https://vk.com/{nick}"
    if check_account_exists(vk_url):
        result["social"].append(f"✅ VK: {vk_url}")
        # Пытаемся получить данные через API
        try:
            vk_api = f"https://api.vk.com/method/users.get?user_ids={nick}&fields=city,bdate,country,sex&v=5.131"
            ...
            if r.status_code == 200:
                ...
                if "response" in data and data["response"]:
                    ...
                    if "bdate" in user:
                        result["personal"]["VK_Дата_рождения"] = user["bdate"]
                    if "country" in user and "title" in user["country"]:
                        result["personal"]["VK_Страна"] = user["country"]["title"]
        except:
            pass

    # 2. Проверяем Instagram
    ig_url = f"https://instagram.com/{nick}"
    if check_account_exists(ig_url):
        result["social"].append(f"✅ Instagram: {ig_url}")

    # 3. Проверяем Telegram
    tg_url = f"https://t.me/{nick}"
    if check_account_exists(tg_url):
        result["social"].append(f"✅ Telegram: {tg_url}")

    # 4. Проверяем TikTok
    tiktok_url = f"https://www.tiktok.com/@{nick}"
    if check_account_exists(tiktok_url):
        result["social"].append(f"✅ TikTok: {tiktok_url}")

    # 5. Проверяем GitHub
    github_url = f"https://github.com/{nick}"
    if check_account_exists(github_url):
        result["social"].append(f"✅ GitHub: {github_url}")

    # 6. Проверяем Twitter
    twitter_url = f"https://twitter.com/{nick}"
    if check_account_exists(twitter_url):
        result["social"].append(f"✅ Twitter: {twitter_url}")

    # 7. Проверяем YouTube
    youtube_url = f"https://www.youtube.com/@{nick}"
    if check_account_exists(youtube_url):
        result["social"].append(f"✅ YouTube: {youtube_url}")

    # 8. Проверяем Wikipedia (известные личности)
    wiki_url = f"https://ru.wikipedia.org/wiki/{clean_query.replace(' ', '_')}"
    if check_account_exists(wiki_url):
        result["social"].append(f"📚 Wikipedia: {wiki_url}")

    # 9. Google и Яндекс только если есть ФИО (2+ слова)
    ...
    if not result["social"]:
        ...
        result["social"].append(f"❌ Ничего не найдено в открытых источниках. Попробуйте уточнить запрос.")
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            ...
            "📱 *Номер телефона*\n"
            "→ Проверка в Telegram, WhatsApp, Viber + утечки\n\n"
            "📧 *Email*\n"
