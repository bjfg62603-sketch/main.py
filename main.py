...
# ===== ПОИСК В WIKIPEDIA С ПАРСИНГОМ СТРУКТУРЫ =====
...
def search_yandex(query):
    try:
        ...
        if r.status_code == 200:
            ...
            if desc_match:
                return re.sub(r'<.*?>', '', desc_match.group(1))[:300]
            alt_match = re.search(r'<span class="organic__text">(.*?)</span>', r.text, re.DOTALL)
            if alt_match:
                return re.sub(r'<.*?>', '', alt_match.group(1))[:300]
    except:
        pass
    return None
...
def get_person_info(query):
    ...
    if ddg_data:
        return {"source": "DuckDuckGo", "text": ddg_data}
    
    # Яндекс
    yandex_data = search_yandex(query)
    if yandex_data:
        return {"source": "Яндекс", "text": yandex_data}
    
    # Google
    google_data = search_google(query)
    if google_data:
        return {"source": "Google", "text": google_data}
...
async def smart_search(query, user_id):
    ...
    if person_info:
        ...
        if "data" in person_info and "description" in person_info["data"]:
            result["description"] = person_info["data"]["description"]
...
'''
    ...
    if data.get("wiki_data") and "name" in data["wiki_data"]:
        html += f'    <div class="item">👤 Имя: {data["wiki_data"]["name"]}</div>\n'
...
# ===== ОБРАБОТЧИК СООБЩЕНИЙ С УЛУЧШЕННЫМ ФОРМАТИРОВАНИЕМ =====
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📌 Бот ищет в Wikipedia, DuckDuckGo, Яндекс, Google.",
        reply_markup=main_menu(),
        parse_mode="Markdown"
    )
