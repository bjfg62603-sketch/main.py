...
async def smart_search(query):
    ...
    if phone_match:
        ...
        result["social"].append(f"🎵 TikTok (поиск по номеру): https://www.tiktok.com/search?q={phone}")
        return result
    ...
    # --- Поиск по ФИО или нику (реальный) ---
    clean_query = re.sub(r'[^\w\s]', '', query).strip()
    words = clean_query.split()
    nick = clean_query.replace(" ", "_")

    # --- Проверяем, есть ли аккаунт в VK ---
    try:
        vk_api_url = f"https://api.vk.com/method/users.search?q={clean_query}&count=1&v=5.131"
        ...
        if r.status_code == 200:
            ...
            if "response" in data and data["response"]["count"] > 0:
                ...
                result["social"].append(f"✅ VK: {vk_link}")
                if "first_name" in user and "last_name" in user:
                    result["personal"]["ФИО"] = f"{user['first_name']} {user['last_name']}"
    except:
        pass

    # --- Проверяем Instagram ---
    try:
        instagram_url = f"https://instagram.com/{nick}"
        r = requests.head(instagram_url, timeout=3)
        if r.status_code == 200:
            result["social"].append(f"📸 Instagram: {instagram_url}")
    except:
        pass

    # --- Проверяем Telegram ---
    try:
        telegram_url = f"https://t.me/{nick}"
        r = requests.head(telegram_url, timeout=3)
        if r.status_code == 200:
            result["social"].append(f"📱 Telegram: {telegram_url}")
    except:
        pass
    ...
    try:
        ...
        r = requests.head(youtube_url, timeout=3)
        if r.status_code == 200:
            result["social"].append(f"🎬 YouTube: {youtube_url}")
    except:
        pass

    # --- Wikipedia (если это известная личность) ---
    try:
    ...
    # --- Новости (Google News) ---
    news_query = f"https://news.google.com/search?q={clean_query.replace(' ', '+')}"
    result["social"].append(f"📰 Google News: {news_query}")

    # --- Видео (YouTube) ---
    video_query = f"https://www.youtube.com/results?search_query={clean_query.replace(' ', '+')}"
    result["social"].append(f"🎬 YouTube (видео): {video_query}")
