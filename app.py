...
ADMIN_IDS = [8428048355, 8164031956]  # @oryke, @shezik
BASE_URL = "https://smertnyteam.onrender.com"  # ТВОЙ URL
...
def send_message(chat_id, text, reply_markup=None, parse_mode="Markdown"):
    ...
    try:
        ...
        logger.info(f"📤 send_message to {chat_id}: {resp.status_code}")
...
def edit_message(chat_id, message_id, text, reply_markup=None, parse_mode="Markdown"):
    ...
    try:
        resp = requests.post(url, json=payload, timeout=10)
        logger.info(f"✏️ edit_message to {chat_id}: {resp.status_code}")
    except Exception as e:
...
def send_invoice(chat_id, tariff_name, price, promo_code):
    ...
    try:
        resp = requests.post(url, json=payload, timeout=10)
        logger.info(f"💳 send_invoice to {chat_id}: {resp.status_code}")
        return resp.json()
    except Exception as e:
...
# Принудительная установка вебхука при первом запросе
webhook_set = False
...
def set_webhook():
    ...
    if not webhook_set:
        ...
        try:
            resp = requests.post(url, json={"url": webhook_url, "drop_pending_updates": True}, timeout=10)
...
@app.route("/")
def index():
    return "🚀 PlutoniumDLL Bot is running! Webhook: " + BASE_URL + WEBHOOK_PATH
...
def stats_page():
    total_users, total_purchases, total_stars, free_promos = get_stats()
    return f"""
    <h2>PlutoniumDLL Stats</h2>
    <ul>
        <li>👥 Пользователей: {total_users}</li>
        <li>💳 Продаж: {total_purchases}</li>
        <li>⭐ Звёзд: {total_stars}</li>
        <li>🎫 Свободных промо: {free_promos}</li>
    </ul>
    """

if __name__ == "__main__":
    logger.info("🚀 Запуск PlutoniumDLL Bot...")
